    ...
    while (true)
    {
        ...
        {                                        // Wait for an event to be received
            ...
            {                                                                        // Check if the event is a data event
                ...
                if (state == DMXStateRx::Break)
                { // Handle a break, reset the state and the current address
                    if (rx_buffer[0] == 0)
                    {
                ...
                if (state == DMXStateRx::Data)
                { // Read the data to the current address in the buffer
                    if (xSemaphoreTake(dmx_rx_mutex, portMAX_DELAY))
                    {
            ...
            else
            {
                ...
                // Events other than DATA (Break, IDLE, Error) usually mean the frame ended.
                if (event.type == UART_BREAK)
                {
                    // If we have a registered callback AND we actually read data in the last frame
                    if (callback && current_rx_addr > 0)
                    {
                        // Pass the pointer and the total bytes read (including Start Code)
                        callback(dmx_data_rx, current_rx_addr);
                    }
                }
...
void EasyDMX::onReceive(DMXReceiveCallback cb)
{
    this->callback = cb;
}
...
void DMXUniverse::removeFixture(uint16_t address)
{
    for (int i = 0; i < fixtures.size();)
    {
        ...
        { // In case of multiple fixtures with the same address this prevents skipping
            i++;
        }
    }
}
